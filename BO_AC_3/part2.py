...
def plotUnknown(unknownName, highName, lowName):
    plt.figure()
    unknown = np.loadtxt("./Unknowns/unknown" + unknownName + ".txt") # importing data from name
    plt.plot(unknown[:,0], unknown[:,1], label="Unknown" + unknownName, color="blue", linestyle="solid")
    high = np.loadtxt("./Standards/" + highName + ".txt") # importing data from name
    low = np.loadtxt("./Standards/" + lowName + ".txt") # importing data from name
...
# Unknown3:
plotUnknown("3", "g6", "g0") # Unknown 3 is determined to be closest to g6 and k0
plotResiduals("3", "g6", "g0") # g6 is better
